...
class auth {
	...
	function check_new_user($login, $passwd, $passwd2, $mail) {
		...
		if ($db->query("SELECT * FROM users WHERE social_id='".$login."';", 'num_row', '')!=0) $error[]='Пользователь с таким именем уже существует';
		if ($db->query("SELECT * FROM users WHERE email='".$mail."';", 'num_row', '')!=0) $error[]='Пользователь с таким email уже существует';
	...
	function reg($login, $passwd, $passwd2, $mail) {
		if (($this->check_new_user($login, $passwd, $passwd2, $mail))=='good') {
			...
			$login = $db->screening($login);
			$dateup = date("Y-m-d");
			if ($db->query("INSERT INTO `users` (`id_user`,`name`, `social_id`, `passwd_user`, `email`,`datereg`,`prev`) VALUES (NULL, '".$login."', '".$mail."', '".$passwd."', '".$mail."','".$dateup."','1');", '', '')) return true;
			else {
	...
	function check() {
		if (isset($_SESSION['id_user']) and isset($_SESSION['social_id'])) return true;
		else {
			...
			if (isset($_COOKIE['id_user']) and isset($_COOKIE['code_user'])) {
				...
				if ($db->query("SELECT * FROM `session` WHERE `id_user`=".$id_user.";", 'num_row', '')==1) {
					...
					if ($data['code_sess']==$code_user and $data['user_agent_sess']==$_SERVER['HTTP_USER_AGENT']) {
						...
						$_SESSION['social_id']=$db->query("SELECT social_id FROM `users` WHERE  `id_user` = '".$id_user."';", 'result', 0);
						//~ обновляем куки
	...
	function authorization() {
		...
		if ($db->query("SELECT * FROM `users` WHERE  `social_id` =  '".$login."' AND  `passwd_user` = '".$passwd."';", 'num_row', '')==1) {
			//~ пользователь найден в бд, логин совпадает с паролем
			$_SESSION['id_user']=$db->query("SELECT * FROM `users` WHERE  `social_id` =  '".$login."' AND  `passwd_user` = '".$passwd."';", 'result', 0);
			$_SESSION['social_id']=$login;
			...
			$dateup = date("Y-m-d");
			  mysql_query( "UPDATE `users` SET `date` = '{$dateup}' WHERE `id_user`=".$_SESSION['id_user']."" );
			...
			if ($db->query("SELECT * FROM  `users` WHERE  `social_id` =  '".$login."';", 'num_row', 0)==1) $error[]='Введен не верный пароль';
	...
	function recovery_pass($login, $mail) {
		$db = new mysql(); //~ создаем новый объект класса
		$login = $db->screening($login);
		$db_inf = $db->query("SELECT * FROM `users` WHERE `social_id`='".$login."';", 'accos', '');
		if ($db->query("SELECT * FROM `users` WHERE `social_id`='".$login."';", 'num_row', '')!=1) {
			...
			if ($mail != $db_inf['email']) $error[]='Введенный email не соответствует введенному при регистрации ';
			if (!isset($error)) {
				//~ восстанавливаем пароль
				$new_passwd = $this->generateCode(8);
				$new_passwd_sql = md5($new_passwd.'lol');
			
			$message = "Вы запросили восстановление пароля на сайте Adatum.ru для учетной записи ".$db_inf['social_id']." \nВаш новый пароль: ".$new_passwd."\n\n С уважением администрация сайта Adatum.ru.";
				
				if (
				mail($mail, "=?utf-8?B?".base64_encode("Восстановление пароля")."?=", $message, "From: info@adatum.ru\r\n"."Content-type: text/plain; charset=\"UTF-8\""."Reply-To: info@adatum.ru\r\n"."X-Mailer: PHP/"."charset=utf-8" . phpversion())
	...
	function error_print($error) {
		$r='<h3>Произошли следующие ошибки:</h3>'."\n".'<ul>';
